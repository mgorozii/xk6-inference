...
var _ = Describe("Inference", func() {
	...
	Context("Inference", func() {
		...
		It("should accept Triton HTTP request body", func() {
			mockTriton := &mockTritonClient{
				inferFn: func(ctx context.Context, modelName string, modelVersion string, inputs []base.InferInput, outputs []base.InferOutput, options *options.InferOptions) (base.InferResult, error) {
					Expect(inputs).To(HaveLen(1))
					Expect(inputs[0].GetName()).To(Equal("input0"))
					Expect(inputs[0].GetData()).To(Equal([]any{float32(1.0), float32(2.0)}))
			...
			res, err := model.Http(map[string]any{
				"inputs": []any{
					map[string]any{
						"name":     "input0",
						"shape":    []any{1.0, 2.0},
		...
		It("should perform gRPC inference", func() {
			...
			res, err := model.Grpc([]any{[]any{1.0, 2.0}})
			Expect(err).NotTo(HaveOccurred())
			Expect(res).To(HaveKey("output0"))
		})

		It("should use dummy data when body is absent (nil)", func() {
			mockTriton := &mockTritonClient{
				inferFn: func(ctx context.Context, modelName string, modelVersion string, inputs []base.InferInput, outputs []base.InferOutput, options *options.InferOptions) (base.InferResult, error) {
		...
		It("should fail fast when body is provided but incomplete (empty map)", func() {
			client.hc = &mockTritonClient{}
			_, err := model.Http(map[string]any{})
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("missing input input0"))
		})

		It("should fail fast when body is provided but incomplete (empty slice)", func() {
			client.hc = &mockTritonClient{}
			_, err := model.Http([]any{})
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("missing input input0"))
		})
